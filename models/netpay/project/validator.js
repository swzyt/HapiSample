...
var RequestModel = {
    ...
    name: Joi.string().max(100).allow(['', null]).description('项目名称'),
...
var ResponseModel = {
    project_id: Joi.number().integer().required().description('项目标识'),
    name: Joi.string().max(100).allow(['', null]).description('项目名称'),
    alias: Joi.string().max(100).allow(['', null]).description('别名'),
    contact_name: Joi.string().max(100).allow(['', null]).description('联系人名称'),
    contact_mobile: Joi.string().max(100).allow(['', null]).description('联系人电话'),
...
module.exports = {
    ...
    get: {
        request: {
            params: {
                project_id: Joi.string().required().description('项目标识')
            }
        },
        response: {
            schema: Joi.object({
    ...
    list: {
        request: {
            query: {
                name: Joi.string().allow(['', null]).description('项目名称'),
                description: Joi.string().allow(['', null]).description('描述'),
    ...
    create: {
        ...
        response: {
            schema: Joi.object({
                code: Joi.number().integer().description("返回代码"),
                message: Joi.string().description('返回信息'),
                data: Joi.object(ResponseModel).meta({ className: PREFIX + "PostResponseData" }).required().description("项目信息")

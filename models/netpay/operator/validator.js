...
var RequestModel = {
    operator_id: Joi.number().integer().optional().description('运营商标识'),
    name: Joi.string().max(100).allow(['', null]).description('运营商名称'),
    line_number: Joi.string().max(100).allow(['', null]).description('线路编号'),
...
var ResponseModel = {
    operator_id: Joi.number().integer().required().description('运营商标识'),
    name: Joi.string().max(100).allow(['', null]).description('运营商名称'),
    line_number: Joi.string().max(100).allow(['', null]).description('线路编号'),
    contact_name: Joi.string().max(100).allow(['', null]).description('联系人名称'),
    contact_mobile: Joi.string().max(100).allow(['', null]).description('联系人电话'),
...
module.exports = {
    ...
    get: {
        request: {
            params: {
                operator_id: Joi.string().required().description('运营商标识')
            }
        },
        response: {
            schema: Joi.object({
    ...
    list: {
        request: {
            query: {
                name: Joi.string().allow(['', null]).description('运营商名称'),
                description: Joi.string().allow(['', null]).description('描述'),
    ...
    create: {
        ...
        response: {
            schema: Joi.object({
                code: Joi.number().integer().description("返回代码"),
                message: Joi.string().description('返回信息'),
                data: Joi.object(ResponseModel).meta({ className: PREFIX + "PostResponseData" }).required().description("运营商信息")

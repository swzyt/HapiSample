...
var RequestModel = {
    ...
    building_id: Joi.number().integer().optional().description('楼栋标识'),
    name: Joi.string().max(100).allow(['', null]).description('楼栋名称'),
...
var ResponseModel = {
    building_id: Joi.number().integer().required().description('楼栋标识'),
    name: Joi.string().max(100).allow(['', null]).description('楼栋名称'),
    alias: Joi.string().max(100).allow(['', null]).description('别名'),
    description: Joi.string().allow(['', null]).description('描述'),
    valid: Joi.boolean().allow(['', null]).description('是否有效'),
...
module.exports = {
    ...
    get: {
        request: {
            params: {
                building_id: Joi.string().required().description('楼栋标识')
            }
        },
        response: {
            schema: Joi.object({
    ...
    list: {
        request: {
            query: {
                name: Joi.string().allow(['', null]).description('楼栋名称'),
                description: Joi.string().allow(['', null]).description('描述'),
    ...
    create: {
        ...
        response: {
            schema: Joi.object({
                code: Joi.number().integer().description("返回代码"),
                message: Joi.string().description('返回信息'),
                data: Joi.object(ResponseModel).meta({ className: PREFIX + "PostResponseData" }).required().description("楼栋信息")

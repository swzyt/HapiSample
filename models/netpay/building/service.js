...
var Service = function (db) {
    ...
    this.attributes = ['building_id', 'name', 'alias', 'description', 'valid', 'created_at', 'updated_at', 'project_id'];
};
//普通列表
Service.prototype.list = function (where, page_size, page_number, orderArr) {
    ...
    return this.db.NetPayBuilding.findAndCountAll(options)
};
//获取单项
Service.prototype.get = function (where) {
    ...
    return this.db.NetPayBuilding.findOne(option);
};
//创建
Service.prototype.create = function (data) {
    ...
    return self.db.NetPayBuilding.build(data).save().then(result => {
        if (result) {
...
Service.prototype.delete = function (where) {
    ...
    return this.db.NetPayBuilding.findOne({ where: where }).then(function (item) {
        if (item)
            return item.destroy();
        else
...
Service.prototype.update = function (where, data) {
    return this.db.NetPayBuilding.update(data, { where: where });
};

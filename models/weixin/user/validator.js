...
const PREFIX = "WeixinUser";

var RequestModel = {
    wx_user_id: Joi.number().integer().optional().description('微信用户标识'),
    name: Joi.string().max(100).allow(['', null]).description('微信用户名称'),
    alias: Joi.string().max(100).allow(['', null]).description('别名'),
    contact_name: Joi.string().max(100).allow(['', null]).description('联系人名称'),
    contact_mobile: Joi.string().max(100).allow(['', null]).description('联系人电话'),
    address: Joi.string().max(100).allow(['', null]).description('地址'),
    description: Joi.string().allow(['', null]).description('描述'),
    valid: Joi.boolean().allow(['', null]).description('是否有效'),
};

var ResponseModel = {
    wx_user_id: Joi.number().integer().required().description('微信用户标识'),
    name: Joi.string().max(100).allow(['', null]).description('微信用户名称'),
    alias: Joi.string().max(100).allow(['', null]).description('别名'),
    contact_name: Joi.string().max(100).allow(['', null]).description('联系人名称'),
    contact_mobile: Joi.string().max(100).allow(['', null]).description('联系人电话'),
...
module.exports = {
    ...
    get: {
        request: {
            params: {
                wx_user_id: Joi.string().required().description('微信用户标识')
            }
        },
        response: {
            schema: Joi.object({
    ...
    list: {
        request: {
            query: {
                name: Joi.string().allow(['', null]).description('微信用户名称'),
                description: Joi.string().allow(['', null]).description('描述'),
    ...
    create: {
        ...
        response: {
            schema: Joi.object({
                code: Joi.number().integer().description("返回代码"),
                message: Joi.string().description('返回信息'),
                data: Joi.object(ResponseModel).meta({ className: PREFIX + "PostResponseData" }).required().description("微信用户信息")
